	...
	var methods = {
		...
		imageWidget: function(){
			...
			var getURL = $("<button/>",{
				...
				if(url ==''){
					methods.showMessage.apply(this,["imgErrMsg_" + _idSuffix,"Пожалуйста, введите url изображения"]);
					return false;
				}
				...
				var image = $('<img/>',{
					src:url,
				  	...
				  	methods.showMessage.apply(this,["imgErrMsg_" + _idSuffix,"Неверный url изображения"]);
				  	return false;
				...
				class:"row-fluid"
					...
					id:"tblForm"+idExtn 
						...
						class:"row" 
							...
							}).append($('<label/>',{ for:"tblRows"+idExtn,	text:"Строки"}
							)).append($('<input/>',{
								id:"tblRows"+idExtn,
								type:"text",
								class:"form-control form-control-width",
								value:2
							})).append($('<label/>',{ for:"tblColumns"+idExtn,	text:"Колонки"}
							)).append($('<input/>',{
								id:"tblColumns"+idExtn,
								type:"text",
							 	class:"form-control form-control-width",
							 	value:2
							})).append($('<label/>',{ for:"tblWidth"+idExtn, text:"Ширина"}
							)).append($('<input/>',{
								id:"tblWidth"+idExtn,
								type:"text",
								class:"form-control form-control-width",
								value:400
							})).append($('<label/>',{ for:"tblHeight"+idExtn, text:"Высота"}
							)).append($('<input/>',{ 
								id:"tblHeight"+idExtn,
								type:"text",
								class:"form-control form-control-width", 
							...
							class:"col-xs-12 col-sm-6 col-md-6 col-lg-6"
							}).append($('<label/>',{ for:"tblAlign"+idExtn, text:"Выравнивание"}
							)).append($('<select/>',{ id:"tblAlign"+idExtn, class:"form-control form-control-width"}
								).append($('<option/>',{ text:"Выбрать", value:""}
								)).append($('<option/>',{ text:"Лево", value:"left"}
								)).append($('<option/>',{ text:"Центр", value:"center"}
								)).append($('<option/>',{ text:"Право",	value:"right"}))
		...
		init : function( options )
		{
			...
			var menuItems = { 'fonteffects': true,
							 ...
							 'font_size': {	"select":true,
							  ...
							  'color'	: { "text":"A",
											"icon": "fa fa-font", 
											"tooltip": "Цвет текста/ фон текста",
											"commandname":null,
						     ...
						     'l_align'	: { "text": "leftalign", 
											...
											"tooltip": "Левое выравнивание", 
											"commandname":"justifyleft", 
											"custom":null },

						     'r_align'	: { "text": "rightalign", 
											"icon": "fa fa-align-right", 
											"tooltip": "Правое выравнивание", 
											"commandname":"justifyright", 
											"custom":null },

						     'c_align'	: { "text": "centeralign", 
											"icon": "fa fa-align-center", 
											"tooltip": "Центральное выравнивание", 
											"commandname":"justifycenter", 
											"custom":null },

						     'justify'	: { "text": "justify", 
											"icon": "fa fa-align-justify", 
											"tooltip": "По всей ширине", 
						...
						'insert_table'	: { "modal": true,
						   ...
						   'indent'		: { "text": "Indent", 
											"icon":"fa fa-indent", 
											"tooltip": "Сместить текст вправо", 
											"commandname":"indent", 
											"custom":null },

						   'outdent'	: { "text": "Outdent", 
											"icon":"fa fa-outdent", 
											"tooltip": "Сместить текст влево", 
											"commandname":"outdent", 
											"custom":null },
							...
							'rm_format'	: { "text": "Remove format", 
											"icon":"fa fa-eraser", 
											"tooltip": "Удалить форматирование текста", 
		...
		createTableContext: function(event,cMenuUl){
			...
			cMenuUl.append($('<li/>',{class:"dropdown-submenu",css:{display:"block"}})
   						...
   						.append($('<ul/>',{class:"dropdown-menu"})
   								...
   								.append($('<li/>').append($('<a/>',{text:"Удалить колонку"}).click( 
										function(e) { return function(){ 
												$('#context-menu').remove();
												var selectedCell = $(e.target);
												var columnIndex = selectedCell.siblings().addBack().index(selectedCell);
			...
			cMenuUl.append($('<li/>').append(modalTrigger))
					...
					.append($('<li/>').append($('<a/>',{text:"Удалить таблицу"}).click( 
						function(e){ return function(){ 
								$('#context-menu').remove();
